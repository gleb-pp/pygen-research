...
Здесь и возникает проблема: при использовании модуля `dill`, который является расширением стандартного модуля `pickle` и повсеместно используется для сериализации объектов в Python, при попытке сериализации генератора возникает ошибка `TypeError: cannot pickle 'generator' object`. Вы можете изучить описанный сценарий в файле `research/benchmarks/real.py` или посмотреть на более простой пример в файле `research/benchmarks/simple.py`.
...
pip install poetry
poetry install
poetry run python research/benchmarks/simple.py
```

После изучения нескольких статей и обсуждения с нейросетями, я получил ответ о том, что генераторы в Python не поддерживают сериализацию, и что это ограничение не конкретного модуля `dill`, а структуры генераторов в Python в целом. Тогда я подумал, что это довольно смелое заявление, ведь генераторы — это объекты, и если `dill` может сериализовать функции, классы и даже замыкания, то в чем особенность генераторов? Это заставило меня глубже погрузиться в этот вопрос и изучить их внутреннюю структуру.
...
- `FRAME_CREATED`: Фрейм создан, но не инициализирован
...
- `FRAME_CLEARED`: Фрейм завершен, очищен
...
- фрейм создается, но он пока не инициализируется 
...
Данный подход будет работать, однако в текущей реализации:
...
Обработка `yield from` происходит следующим образом:
...
- `PyCoro_Type` имеет структуру схожую с `PyGen_Type`, но с другими префиксами полей (`cr_` вместо `gi_`)
- Python определяет корутину через флаг `CO_COROUTINE`, который устанавливается компилятором при встрече `async def` без `yield`
- await-команды "под капотом" работают через `yield from`
...
Вы можете ознакомиться с материалами бенчмарков в директории `research/benchmarks/lists/`.

| Время создания (сравнение) |
| --- |
| <img src="research/benchmarks/lists/comparison_creation_time.png" width="400"/> |

На графике видно, что время создания списка линейно растет с увеличением количества элементов, тогда как время создания генератора остается постоянным, сильно меньше времени создания списка. Во время создания список инициализирует все элементы, тогда как генератор только создает структуру и выделяет память для фрейма.

| Использование памяти (сравнение) | Использование памяти (генератор) |
| --- | --- |
| <img src="research/benchmarks/lists/comparison_memory_usage.png" width="400"/> | <img src="research/benchmarks/lists/generator_memory_usage.png" width="400"/> |
...
На графике также можно заметить, как потребление памяти списком растет "ступенями", что показывает, как память в CPython выделяется блоками, для оптимизации работы с памятью и уменьшения фрагментации.

| Время итерации (сравнение) |
| --- |
| <img src="research/benchmarks/lists/comparison_iteration_time.png" width="400"/> |

На графииках видно, что время итерации и по списку и по генератору линейно растет с увеличением количества элементов, однако итерация по генератору занимает в несколько раз больше времени. Это связано с тем, что при итерации по генератору происходит переключение контекста между фреймом генератора и вызывающим кодом, а также дополнительные проверки состояния генератора, что добавляет накладные расходы. В случае списка итерация происходит напрямую по сгенерированным заранее элементам без дополнительных переключений контекста.

### Генератор vs Итератор-класс

Как мы обсуждали выше, до появления генераторов в Python для создания итераторов приходилось писать классы с методами `__iter__` и `__next__`, а также хранить состояние в атрибутах класса. В этом эксперименте я сравниваю время создания и итерации по встроенному генератору и кастомному итератор-классу, который реализует аналогичный функционал.
...
Вы можете ознакомиться с материалами бенчмарков в директории `research/benchmarks/iterator/`.

| Время выполнения (сравнение) |
| --- |
| <img src="research/benchmarks/iterator/comparison_execution_time.png" width="400"/> |
...
С увеличением глубины вложенности генераторов ухудшается производительность из-за увеличения количества переключений контекста и управления состоянием. В этом эксперименте я сравниваю время выполнения цепочки генераторов, использующих `yield from`, с ручной реализацией обхода вложенных генераторов через цикл.

Вы можете ознакомиться с материалами бенчмарков в директории `research/benchmarks/yield_from/`.

| Время выполнения (сравнение) |
| --- |
| <img src="research/benchmarks/yield_from/comparison_execution_time.png" width="400"/> |

На графике видно, что время выполнения обеих реализаций линейно растет с увеличением глубины вложенности, однако реализация с `yield from` работает примерно в 1.5 раза быстрее. Это связано с тем, что `yield from` позволяет делегировать выполнение напрямую подгенератору, избегая дополнительных циклов на уровне Python и уменьшая количество переключений контекста, что улучшает производительность.
...
Проведённые эксперименты показывают, что генераторы не являются универсальной заменой другим конструкциям. В простых и вычислительно интенсивных задачах обычные функции и встроенные операции оказываются быстрее за счёт отсутствия накладных расходов на `yield` и переключение контекста. В то же время списки удобны, когда важна мгновенная доступность всех данных и нет жёстких ограничений по памяти.

Генераторы же наиболее эффективны в задачах потоковой обработки и при работе с большими или потенциально бесконечными данными, где критична экономия памяти и возможность пошагового выполнения. Однако в системах, требующих сохранения состояния между запусками, их использование ограничено, и предпочтение стоит отдавать явным итераторам или другим сериализуемым структурам. Таким образом, каждый инструмент занимает свою нишу, и выбор между ними должен определяться требованиями конкретной задачи.
